//
//  FSLAccountLoginViewController.m
//  WeChat
//
...
#import "FSLAccountLoginViewController.h"
#import "FSLLoginCaptchaView.h"
#import "FSLLoginPasswordView.h"
#import "FSLButton.h"
@interface FSLAccountLoginViewController ()
/// viewModel
@property (nonatomic, readonly, strong) FSLAccountLoginViewModel *viewModel;
/// accountBtn
@property (nonatomic, readwrite, weak) FSLButton *accountBtn;
/// loginBtn
...
/// passwordView
@property (nonatomic, readwrite, weak) FSLLoginPasswordView *passwordView;
/// captchaView
@property (nonatomic, readwrite, weak) FSLLoginCaptchaView *captchaView;
@end

@implementation FSLAccountLoginViewController
...
- (void)bindViewModel{
    ...
    [[[self.changeLoginBtn rac_signalForControlEvents:UIControlEventTouchUpInside]
     ...
     subscribeNext:^(UIButton *sender) {
         ...
         CGFloat offsetX1 = sender.isSelected?FSL_SCREEN_WIDTH:0;
         CGFloat offsetX2 = sender.isSelected?0:FSL_SCREEN_WIDTH;
         /// 做动画
    ...
    [[self.loginBtn rac_signalForControlEvents:UIControlEventTouchUpInside]
     subscribeNext:^(UIButton *sender) {
         ...
         if (self.changeLoginBtn.isSelected) { /// 手机号登录
             /// 验证码 六位
             if(self.captchaView.captchaTextField.text.length != 6 || ![NSString fsl_isPureDigitCharacters:self.viewModel.captcha]){
                 [NSObject fsl_showAlertViewWithTitle:@"验证码超时，请重新获取验证码。" message:nil confirmTitle:@"确定"];
                 return ;
             }
         }else{
             /// 密码 8~16位 且 不能含有中文
             if(self.passwordView.passwordTextField.text.length < 8
                ...
                || [NSString fsl_isContainChinese:self.passwordView.passwordTextField.text] ){
                 [NSObject fsl_showAlertViewWithTitle:@"账号或密码错误，请重新填写。" message:nil confirmTitle:@"确定"];
                 return ;
             }
         }
    ...
    [[self.captchaView.captchaBtn rac_signalForControlEvents:UIControlEventTouchUpInside]
     subscribeNext:^(UIButton *sender) {
         ...
         [NSObject fsl_showAlertViewWithTitle:@"确认手机号码" message:message confirmTitle:@"好的" cancelTitle:@"取消" confirmAction:^{
             @strongify(self);
...
- (void)_setup{
    ...
    /// 适配
    FSLAdjustsScrollViewInsets_Never(scrollView);
}

#pragma mark - 设置导航栏
- (void)_setupNavigationItem{
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem fsl_systemItemWithTitle:nil titleColor:nil imageName:@"barbuttonicon_more_black_30x30" target:self selector:@selector(_moreItemDidClicked) textType:NO];
}
...
- (void)_setupSubViews{
    ...
    avatarView.image = [UIImage fsl_imageAlwaysShowOriginalImageWithImageName:@"SettingProfileHead_66x66"];
    avatarView.layer.cornerRadius = 6.0f;
    avatarView.layer.masksToBounds = YES;
    avatarView.layer.borderWidth = 1.0f;
    avatarView.layer.borderColor = FSLColorFromHexString(@"#E4E4E4").CGColor;
    [containerView addSubview:avatarView];
    self.avatarView = avatarView;
    
    /// 账号
    FSLButton *accountBtn = [[FSLButton alloc] init];
    accountBtn.titleLabel.font = FSLMediumFont(17.0f);
    ...
    /// passwordView
    FSLLoginPasswordView *passwordView = [FSLLoginPasswordView passwordView];
    passwordView.backgroundColor = containerView.backgroundColor;
    [passwordBaseView addSubview:passwordView];
    self.passwordView = passwordView;
    
    /// captchaView
    FSLLoginCaptchaView *captchaView = [FSLLoginCaptchaView captchaView];
    ...
    [changeLoginBtn setTitleColor:FSLColorFromHexString(@"#576B95") forState:UIControlStateNormal];
    changeLoginBtn.titleLabel.font = FSLRegularFont_18;
    ...
    [loginBtn setTitleColor:FSLColorFromHexString(@"#DBF3D8") forState:UIControlStateDisabled];
    /// 设置背景
    [loginBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#51AA38")] forState:UIControlStateNormal];
    [loginBtn setBackgroundImage:[UIImage yy_imageWithColor:[FSLColorFromHexString(@"#51AA38") colorWithAlphaComponent:.5f]] forState:UIControlStateHighlighted];
    [loginBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#AFDDA7")] forState:UIControlStateDisabled];
    loginBtn.layer.cornerRadius= 6.0f;
    loginBtn.layer.masksToBounds = YES;
    loginBtn.layer.borderWidth = 1;
    loginBtn.layer.borderColor = FSLColorFromHexString(@"#499932").CGColor;
    ...
    [containerView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(FSL_SCREEN_HEIGHT);
        make.width.mas_equalTo(FSL_SCREEN_WIDTH);
    }];
    ...
    [captchaView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(passwordBaseView).with.offset(FSL_SCREEN_WIDTH);
        make.top.and.bottom.equalTo(passwordBaseView);
        make.width.equalTo(passwordBaseView);
    }];
...
-(void) _configureData{
    ...
    /// 获取沙盒里面的用户数据
    FSLUser *user = self.viewModel.services.client.currentUser;
    if (user) { /// 配置数据
        ...
        /// 设置头像
        [self.avatarView yy_setImageWithURL:user.profileImageUrl placeholder:[UIImage fsl_imageAlwaysShowOriginalImageWithImageName:@"SettingProfileHead_66x66"] options:FSLWebImageOptionAutomatic completion:NULL];
        /// 这里假设  只有两种方式 （qq登录 + 手机号登录）
        self.viewModel.account = user.qq;
        self.viewModel.phone = user.phone;
        
        /// 判断一下登录渠道 增强用户体验
        if (user.channel == FSLUserLoginChannelTypePhone) { /// 手机号登录
            ...
            CGFloat offsetX1 = self.changeLoginBtn.isSelected?FSL_SCREEN_WIDTH:0;
            CGFloat offsetX2 = self.changeLoginBtn.isSelected?0:FSL_SCREEN_WIDTH;
            /// 更新约束
            [self.passwordView mas_updateConstraints:^(MASConstraintMaker *make) {

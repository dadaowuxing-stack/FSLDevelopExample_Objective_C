//
//  FSLMobileLoginViewController.m
//  WeChat
//
...
#import "FSLMobileLoginViewController.h"
#import "FSLLoginCaptchaView.h"
#import "FSLLoginPasswordView.h"

@interface FSLMobileLoginViewController ()
/// viewModel
@property (nonatomic, readonly, strong) FSLMobileLoginViewModel *viewModel;
...
@implementation FSLMobileLoginViewController
...
- (IBAction)_loginBtnDidClicked:(UIButton *)sender {
    ...
    if (self.changeLoginBtn.isSelected) { /// 手机号登录
        ...
        /// 密码 8~16位 且 不能含有中文
        if(![NSString fsl_isValidMobile:self.viewModel.phone]
...
- (void)_setupNavigationItem{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem fsl_systemItemWithTitle:nil titleColor:nil imageName:@"WAWeb_Back_Dark_10x18" target:nil selector:nil textType:NO];
...
- (void)_setupSubViews{
    /// 设置背景
    [self.loginBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#51AA38")] forState:UIControlStateNormal];
    [self.loginBtn setBackgroundImage:[UIImage yy_imageWithColor:[FSLColorFromHexString(@"#51AA38") colorWithAlphaComponent:.5f]] forState:UIControlStateHighlighted];
    [self.loginBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#AFDDA7")] forState:UIControlStateDisabled];
    ...
    make.left.equalTo(self.passwordBaseView.mas_left).with.offset(FSL_SCREEN_WIDTH);
        make.top.and.bottom.equalTo(self.passwordBaseView);
        make.width.equalTo(self.passwordBaseView);
    }];
}

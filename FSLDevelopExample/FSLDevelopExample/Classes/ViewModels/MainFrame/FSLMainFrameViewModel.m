...
@interface FSLMainFrameViewModel ()
/// 商品数组 <FSLLiveRoom *>
@property (nonatomic, readwrite, copy) NSArray *liveRooms;
@end


@implementation FSLMainFrameViewModel
...
- (void)initialize
{
    ...
    self.didSelectCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSIndexPath * indexPath) {
        @strongify(self);
        /// 这里只是测试
        FSLLiveRoom *liveRoom = self.liveRooms[indexPath.row];
...
- (NSArray *)dataSourceWithLiveRooms:(NSArray *)liveRooms {
    if (FSLObjectIsNil(liveRooms) || liveRooms.count == 0) return nil;
    NSArray *viewModels = [liveRooms.rac_sequence map:^(FSLLiveRoom *liveRoom) {
        FSLMainFrameItemViewModel *viewModel = [[FSLMainFrameItemViewModel alloc] initWithLiveRoom:liveRoom];
        return viewModel;
    }].array;
    return viewModels ?: @[] ;
}

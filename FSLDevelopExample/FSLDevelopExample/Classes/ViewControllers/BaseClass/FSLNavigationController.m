...
@interface FSLNavigationController ()
/// 导航栏分隔线
@property (nonatomic , weak , readwrite) UIImageView * navigationBottomLine;
@end

@implementation FSLNavigationController
...
- (void)_setupNavigationBarBottomLine{
    ...
    UIImageView *navSystemLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.navigationBar.height - navSystemLineH, FSL_SCREEN_WIDTH, navSystemLineH)];
    navSystemLine.backgroundColor = FSLColor(223.0f, 223.0f, 221.0f);
...
+ (void) _setupNavigationBarTheme{
    ...
    textAttrs[NSFontAttributeName] = FSLMediumFont(18.0f);
...
+ (void)_setupBarButtonItemTheme{
    ...
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = [UIColor whiteColor];
    textAttrs[NSFontAttributeName] = FSLRegularFont(fontSize);
    NSShadow *shadow = [[NSShadow alloc] init];
    shadow.shadowOffset =  CGSizeZero;
    textAttrs[NSShadowAttributeName] = shadow;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (self.viewControllers.count > 0){
        ...
        if ([viewController isKindOfClass:[FSLViewController class]]) {
            
            FSLViewModel *viewModel = [(FSLViewController *)viewController viewModel];
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem fsl_backItemWithTitle:title imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_back)];
    }
    // push

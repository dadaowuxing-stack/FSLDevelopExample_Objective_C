...
{
    // 2014-05-01
    NSDate *nowDate = [[NSDate date] fsl_dateWithYMD];
    
    // 2014-04-30
    NSDate *selfDate = [self fsl_dateWithYMD];
...
- (BOOL) fsl_isSameWeekWithAnotherDate: (NSDate *)anotherDate
{
    ...
    return (fabs([self timeIntervalSinceDate:anotherDate]) < FSL_D_WEEK);
}
...
- (BOOL) fsl_isThisWeek
{
    return [self fsl_isSameWeekWithAnotherDate:[NSDate date]];
}
...
{
    return [[NSDateFormatter fsl_defaultDateFormatter] dateFromString:timestamp];
}
...
{
    return [[NSDateFormatter fsl_defaultDateFormatter] stringFromDate:[NSDate date]];
}
...
- (NSString *)fsl_formattedDateDescription
{
    ...
    if ([self fsl_isThisYear])
    {
        // 今年
        if ([self fsl_isToday]) {
            // 22:22
            dateFormatter = [NSDateFormatter fsl_dateFormatterWithFormat:@"HH:mm"];
        }else if ([self fsl_isYesterday]){
            // 昨天 22:22
            dateFormatter = [NSDateFormatter fsl_dateFormatterWithFormat:@"昨天 HH:mm"];
        }else if ([self fsl_isThisWeek]){
            // 星期二 22:22
            dateFormatter = [NSDateFormatter fsl_dateFormatterWithFormat:[NSString stringWithFormat:@"%@ HH:mm" , [self fsl_weekDay]]];
        }else{
            // 2016年08月18日 22:22
            dateFormatter = [NSDateFormatter fsl_dateFormatterWithFormat:@"yyyy年MM月dd日 HH:mm"];
        }
        
    }else{
        // 非今年
        dateFormatter = [NSDateFormatter fsl_dateFormatterWithFormat:@"yyyy年MM月dd日"];
    }
...
- (NSString *)fsl_string_yyyy_MM_dd {
    return [self fsl_string_yyyy_MM_dd:[NSDate date]];
}

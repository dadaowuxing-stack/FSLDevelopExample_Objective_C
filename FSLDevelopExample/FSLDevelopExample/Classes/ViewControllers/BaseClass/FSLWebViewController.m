...
/// KVO 监听的属性
/// 加载情况
static NSString * const FSLWebViewKVOLoading = @"loading";
/// 文章标题
static NSString * const FSLWebViewKVOTitle = @"title";
/// 进度
static NSString * const FSLWebViewKVOEstimatedProgress = @"estimatedProgress";

@interface FSLWebViewController ()
/// webView
...
/// viewModel
@property (nonatomic, strong, readonly) FSLWebViewModel *viewModel;

@end

@implementation FSLWebViewController
{
    /// KVOController 监听数据
...
- (void)viewDidLoad {
    ...
    WKWebView *webView = [[WKWebView alloc] initWithFrame:FSL_SCREEN_BOUNDS configuration:configuration];
    webView.navigationDelegate = self;
    webView.UIDelegate = self;
    
    if ((FSLIOSVersion >= 9.0)) webView.customUserAgent = userAgent;
    ...
    [_KVOController fsl_observe:self.webView keyPath:FSLWebViewKVOEstimatedProgress block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        @strongify(self);
    ...
    if(self.viewModel.shouldPullDownToRefresh){
        [self.webView.scrollView fsl_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            @strongify(self);
            [self.webView reload];
        }];
...
- (UIEdgeInsets)contentInset{
    return UIEdgeInsetsMake(FSL_APPLICATION_TOP_BAR_HEIGHT, 0, 0, 0);
}
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler {
    [NSObject fsl_showAlertViewWithTitle:nil message:message confirmTitle:@"我知道了"];
    completionHandler();
}
...
- (UIProgressView *)progressView {
    if (!_progressView) {
        CGFloat progressViewW = FSL_SCREEN_WIDTH;
        ...
        progressView.progressTintColor = FSL_MAIN_TINTCOLOR;
...
{
    if (_backItem == nil) {
        _backItem = [UIBarButtonItem fsl_backItemWithTitle:@"返回" imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_backItemDidClicked)];
    }
    return _backItem;
}
...
- (UIBarButtonItem *)closeItem {
    if (!_closeItem) {
        _closeItem = [UIBarButtonItem fsl_systemItemWithTitle:@"关闭" titleColor:nil imageName:nil target:self selector:@selector(_closeItemDidClicked) textType:YES];
    }
    return _closeItem;
}

...
@interface FSLLanguageViewModel ()
/// closeCommand
@property (nonatomic, readwrite, strong) RACCommand *closeCommand;
/// 选中的ViewModel
@property (nonatomic, readwrite, strong) FSLLanguageItemViewModel *selectedItemViewModel;
/// 语言ID
...
- (void)initialize{
    ...
    self.didSelectCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSIndexPath * indexPath) {
        ...
        FSLLanguageItemViewModel *itemViewModel = self.dataSource[indexPath.row];
    ...
    self.validCompleteSignal = [RACObserve(self, selectedItemViewModel) map:^id(FSLLanguageItemViewModel * itemViewModel) {
        @strongify(self);
...
- (void)_configureData{
    ...
    for (NSInteger i = 1000; i<(titles.count + 1000); i++) {
        FSLLanguageItemViewModel *itemViewModel = [FSLLanguageItemViewModel itemViewModelWithIdstr:[NSString stringWithFormat:@"%zd",i] title:titles[i-1000]];

...
@implementation FSLEmoticonManager
...
+ (NSArray<FSLEmoticonGroup *> *)emoticonGroups{
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[FSLEmoticonGroup modelArrayWithJSON:packages];
        NSMutableDictionary *groupDic = [NSMutableDictionary new];
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            FSLEmoticonGroup *group = groups[i];
        ...
        for (NSString *path in additionals) {
            FSLEmoticonGroup *group = groupDic[path];
            if (!group) continue;
            ...
            FSLEmoticonGroup *addGroup = [FSLEmoticonGroup modelWithJSON:infoJSON];
            if (addGroup.emoticons.count) {
                for (FSLEmoticon *emoticon in addGroup.emoticons) {

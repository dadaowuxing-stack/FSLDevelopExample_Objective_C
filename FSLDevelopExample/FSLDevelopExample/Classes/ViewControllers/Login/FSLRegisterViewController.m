...
#import "FSLRegisterViewController.h"
#import "FSLMobileLoginView.h"

@interface FSLRegisterViewController ()
/// viewModel
@property (nonatomic, readwrite, strong) FSLRegisterViewModel *viewModel;
/// containerView
...
@implementation FSLRegisterViewController
...
- (IBAction)_registerBtnDidClicked:(UIButton *)sender {
    ...
    if (![NSString fsl_isValidMobile:self.viewModel.phone]) {
        [NSObject fsl_showAlertViewWithTitle:@"手机号码错误" message:@"你输入的是一个无效的手机号码" confirmTitle:@"确定"];
        return;
    }
...
- (void)_setupNavigationItem{
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem fsl_systemItemWithTitle:@"取消" titleColor:FSL_MAIN_TINTCOLOR imageName:nil target:nil selector:nil textType:YES];
...
- (void)_setupSubViews{
    /// 设置背景
    [self.registerBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#51AA38")] forState:UIControlStateNormal];
    [self.registerBtn setBackgroundImage:[UIImage yy_imageWithColor:[FSLColorFromHexString(@"#51AA38") colorWithAlphaComponent:.5f]] forState:UIControlStateHighlighted];
    [self.registerBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLColorFromHexString(@"#AFDDA7")] forState:UIControlStateDisabled];

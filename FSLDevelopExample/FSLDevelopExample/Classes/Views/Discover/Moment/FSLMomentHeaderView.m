...
/// 配图View
@property (nonatomic, readwrite, weak) FSLMomentPhotosView *photosView;
/// 分享View
@property (nonatomic, readwrite, weak) FSLMomentShareInfoView *shareInfoView;
/// videoView
@property (nonatomic, readwrite, weak) FSLMomentVideoView *videoView;
/// upArrow
@property (nonatomic, readwrite, weak) UIImageView *upArrowView;
/// 更多操作view
@property (nonatomic, readwrite, weak) FSLMomentOperationMoreView *operationMoreView;
/// viewModel
@property (nonatomic, readwrite, strong) FSLMomentItemViewModel *viewModel;
@end


@implementation FSLMomentHeaderView
/// init
+ (instancetype)headerViewWithTableView:(UITableView *)tableView{
    static NSString *ID = @"MomentHeader";
    FSLMomentHeaderView *header = [tableView dequeueReusableHeaderFooterViewWithIdentifier:ID];
    if (header == nil) {
        // 缓存池中没有, 自己创建
...
- (void)bindViewModel:(FSLMomentItemViewModel *)viewModel{
    ...
    [self.avatarView yy_setImageWithURL:viewModel.moment.user.profileImageUrl placeholder:FSLDefaultAvatar(FSLDefaultAvatarTypeDefualt) options:YYWebImageOptionAllowInvalidSSLCertificates|YYWebImageOptionAllowBackgroundTask completion:nil];
    /// 昵称
    ...
    /// 分享
    self.shareInfoView.hidden = !(viewModel.moment.type == FSLMomentExtendTypeShare);
    self.shareInfoView.frame = viewModel.shareInfoViewFrame;
    [self.shareInfoView bindViewModel:viewModel];
    
    /// 视频
    self.videoView.hidden = !(viewModel.moment.type == FSLMomentExtendTypeVideo);
    ...
    /// 更多View
    self.operationMoreView.right = self.operationMoreBtn.left - FSLMomentContentInnerMargin;
    self.operationMoreView.centerY = self.operationMoreBtn.centerY;
    self.operationMoreView.width = 0;
...
- (void)_dealWithAction{
    ...
    /// 更多View的各项操作
    /// 点赞
    self.operationMoreView.attitudesClickedCallback = ^(FSLMomentOperationMoreView *operationMoreView) {
        @strongify(self);
        /// 执行点赞
    ...
    /// 评论
    self.operationMoreView.commentClickedCallback = ^(FSLMomentOperationMoreView *operationMoreView) {
        @strongify(self);
        [self.viewModel.commentSubject sendNext:@(self.section)];
    };

    /// 分享View的点击事件
    self.shareInfoView.touchBlock = ^(FSLMomentBackgroundView *view) {
        @strongify(self);
...
- (void)_setupSubViews{
    ...
    contentLable.preferredMaxLayoutWidth = FSLMomentCommentViewWidth();
    [self.contentView addSubview:contentLable];
    self.contentLable = contentLable;
    
    /// photosView
    FSLMomentPhotosView *photosView = [[FSLMomentPhotosView alloc] init];
    ...
    /// shareInfoView
    FSLMomentShareInfoView *shareInfoView = [FSLMomentShareInfoView shareInfoView];
    self.shareInfoView = shareInfoView;
    [self.contentView addSubview:shareInfoView];
    
    /// videoView
    FSLMomentVideoView *videoView = [FSLMomentVideoView videoView];
    ...
    [operationMoreBtn setImage:FSLImageNamed(@"wx_albumOperateMore_25x25") forState:UIControlStateNormal];
    [operationMoreBtn setImage:FSLImageNamed(@"wx_albumOperateMoreHL_25x25") forState:UIControlStateHighlighted];
    ...
    [expandBtn setTitleColor:FSLRandomColor forState:UIControlStateNormal];
    [expandBtn.titleLabel setFont:FSLMomentExpandTextFont];
    [expandBtn setBackgroundImage:[UIImage yy_imageWithColor:FSLRandomColor size:CGSizeMake(FSLMomentExpandButtonWidth, FSLMomentExpandButtonHeight)] forState:UIControlStateHighlighted];
    /// 子控件超出部分裁剪
    ...
    /// 向上的箭头
    UIImageView *upArrowView = [[UIImageView alloc] initWithImage:FSLImageNamed(@"wx_albumTriangleB_45x6")];
    self.upArrowView = upArrowView;
    [self.contentView addSubview:upArrowView];
    
    /// 更多操作
    FSLMomentOperationMoreView *operationMoreView = [[FSLMomentOperationMoreView alloc] init];

...
#import "FSLLoginViewController.h"
#import "FSLMobileLoginView.h"
#import "FSLAccountLoginView.h"
#import "FSLMobileLoginViewModel.h"

@interface FSLLoginViewController ()
/// viewModel
@property (nonatomic, readonly, strong) FSLLoginViewModel *viewModel;
/// 切换登录方式
...
/// accountLoginView
@property (nonatomic, readwrite, weak) FSLAccountLoginView *accountLoginView;
/// mobileLoginView
@property (nonatomic, readwrite, weak) FSLMobileLoginView *mobileLoginView;
@end

@implementation FSLLoginViewController
...
- (IBAction)_loginBtnDidClicked:(UIButton *)sender {
    ...
    if(self.changeLoginBtn.isSelected){ /// QQ登录
        if (![NSString fsl_isValidQQ:self.accountLoginView.accountTextField.text]
            || self.accountLoginView.passwordTextField.text.length < 8
            || self.accountLoginView.passwordTextField.text.length >16
            || [NSString fsl_isContainChinese:self.accountLoginView.passwordTextField.text] ) {
        ...
        /// 手机号登录 将区号+手机号码 传递过去
        FSLMobileLoginViewModel *viewModel = [[FSLMobileLoginViewModel alloc] initWithServices:self.viewModel.services params:@{FSLMobileLoginPhoneKey:self.viewModel.phone,FSLMobileLoginZoneCodeKey:self.viewModel.zoneCode}];
...
- (void)_setupNavigationItem{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem fsl_systemItemWithTitle:nil titleColor:nil imageName:@"SystemTipBtnClose_26x26" target:nil selector:nil textType:NO];
...
- (void)_setupSubViews{
    ...
    /// 账号登录
    FSLAccountLoginView *accountLoginView = [FSLAccountLoginView accoutLoginView];
    self.accountLoginView = accountLoginView;
    [self.accountBaseView addSubview:accountLoginView];
    
    /// 手机登录
    FSLMobileLoginView *mobileLoginView = [FSLMobileLoginView mobileLoginView];
    ...
    [accountLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.accountBaseView).with.offset(FSL_SCREEN_WIDTH);
        make.top.and.bottom.equalTo(self.accountBaseView);
        make.width.equalTo(self.accountBaseView);
    }];
}

...
@implementation FSLMomentHelper
...
+ (NSString *)createdAtTimeWithSourceDate:(NSDate *)sourceDate{
    if (FSLObjectIsNil(sourceDate)) return @"";
    // 判断是否为今年
    if (sourceDate.fsl_isThisYear) {
        if (sourceDate.fsl_isToday) { // 今天
            NSDateComponents *cmps = [sourceDate fsl_deltaWithNow];
        ...
        } else if (sourceDate.fsl_isYesterday) { // 昨天
...
+ (void)hideAllPopViewWithAnimated:(BOOL)animated{
    /// 关掉更多View
    [FSLMomentOperationMoreView hideAllOperationMoreViewWithAnimated:animated];
    /// 关闭键盘
    if(FSLSharedAppDelegate.isShowKeyboard){ [[UIApplication sharedApplication].keyWindow endEditing:YES]; }
}
@end

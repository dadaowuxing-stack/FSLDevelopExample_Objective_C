...
fsl_propertyAttributes *fsl_copyPropertyAttributes (objc_property_t property) {
    ...
    fsl_propertyAttributes *attributes = calloc(1, sizeof(fsl_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate fsl_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = fsl_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = fsl_propertyMemoryManagementPolicyRetain;
                break;

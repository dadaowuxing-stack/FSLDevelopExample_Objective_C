...
/// 允许最大选中照片数
static CGFloat const FSLMaxImagesCount = 8;

@implementation FSLPhotoManager
...
+ (void)showPhotoBrowser:(UIViewController *)viewController photos:(NSArray<IDMPhoto *> *)photosArray initialPageIndex:(NSUInteger)initialPageIndex animatedFromView:(UIView *)animatedFromView scaleImage:(UIImage *)scaleImage delegate:(id<IDMPhotoBrowserDelegate>)delegate{
    ...
    if (viewController==nil) viewController = [FSLControllerHelper topViewController];
...
+ (void)fetchPhotosFromCamera:(UIViewController *)viewController allowCrop:(BOOL)allowCrop completion:(void (^)(UIImage *, id))completion{
    viewController = (viewController == nil)?[FSLControllerHelper topViewController]:viewController;
    /// 检查授权
    AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) && iOS7Later) {
        // 无相机权限 做一个友好的提示 ，这个只是针对 iOS8+
        [NSObject fsl_showAlertViewWithTitle:@"无法访问你的相机" message:@"请在iPhone的“设置-隐私-相机”选项中，允许轻空访问你相机" confirmTitle:@"设置" cancelTitle:@"取消" confirmAction:^{
        ...
        [NSObject fsl_showAlertViewWithTitle:@"无法访问你的相册" message:@"请在iPhone的“设置-隐私-相册”选项中，允许轻空访问你相册" confirmTitle:@"设置" cancelTitle:@"取消" confirmAction:^{
        ...
        if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
            ...
            title = @"无法打开你的相机！";
#endif
            [NSObject fsl_showAlertViewWithTitle:title message:nil confirmTitle:@"我知道了"];
        }
    }
}
...
+ (void)configureImagePicker:(TZImagePickerController *)imagePicker
{
    ...
    /// 圆形裁剪size
    imagePicker.circleCropRadius = (FSL_SCREEN_MIN_LENGTH - 60);
    /// 配置cropView
    ...
    NSInteger left = 0;
    NSInteger widthHeight = FSL_SCREEN_WIDTH - 2 * left;
    NSInteger top = (FSL_SCREEN_HEIGHT - widthHeight) / 2;
    /// 裁剪尺寸
...
+ (void) accessApplicationSetting:(BOOL)isAblum{
    if (iOS8Later) {
        if (![[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]]) {
            [NSObject fsl_showAlertViewWithTitle:@"十分抱歉" message:@"无法跳转到隐私设置页面，请手动前往设置页面，谢谢" confirmTitle:@"确定"];
        }
    }
}
...
+ (NSInteger)colunmNumber{
    return (FSL_IS_IPHONE_5||FSL_IS_IPHONE_4_OR_LESS)?3:4;
}

/// 允许最大选择
+ (NSInteger)maxImagesCount{
    return FSLMaxImagesCount;
}

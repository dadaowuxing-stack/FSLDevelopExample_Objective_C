...
FOUNDATION_EXTERN NSString *const FSLViewModelIDKey;
/// 传递导航栏title的key：例如 导航栏的title...
FOUNDATION_EXTERN NSString *const FSLViewModelTitleKey;
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
FOUNDATION_EXTERN NSString *const FSLViewModelUtilKey;
/// 传递webView Request的key：例如 webView request...
FOUNDATION_EXTERN NSString *const FSLViewModelRequestKey;

@protocol FSLViewModelServices;

@interface FSLViewModel : NSObject
...
@property (nonatomic, readonly, strong) id<FSLViewModelServices> services;

...
@interface FSLAccountLoginViewModel ()
/// loginCommand
...
@implementation FSLAccountLoginViewModel

- (void)initialize{
    [super initialize];
    
    @weakify(self);
    void (^doNext)(FSLUser *) = ^(FSLUser *user){
        @strongify(self);
        /// 存储登录账号
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            /// 发通知
            [[NSNotificationCenter defaultCenter] postNotificationName:FSLSwitchRootViewControllerNotification object:nil userInfo:@{FSLSwitchRootViewControllerUserInfoKey:@(FSLSwitchRootViewControllerFromTypeLogin)}];
        });
    };
    ...
    self.loginCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSNumber * selected) {
        ...
        if (!selected.boolValue) { /// QQ登录/微信号/邮箱
            ...
            return [[self _fetchUserInfoWithUserLoginChannel:FSLUserLoginChannelTypeQQ] doNext:doNext];
        }else{ /// 手机号登录
            return [[self _fetchUserInfoWithUserLoginChannel:FSLUserLoginChannelTypePhone] doNext:doNext];
        }
    ...
    self.validLoginSignal = [[RACSignal
                              ...
                              reduce:^(NSString *account, NSString *password , NSString *phone , NSString * captcha, NSNumber *selected) {
                                  if (!selected.boolValue) {
                                      return @(FSLStringIsNotEmpty(account) && FSLStringIsNotEmpty(password));
                                  }else{
                                      return @(FSLStringIsNotEmpty(phone) && FSLStringIsNotEmpty(captcha));
                                  }
                              }]
    ...
    self.moreCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSNumber * index) {
        @strongify(self);
        FSLViewModel *viewModel = nil;
        if (index.integerValue == 1) {       /// 切换账号
            viewModel = [[FSLLoginViewModel alloc] initWithServices:self.services params:nil];
            ...
            viewModel = [[FSLRegisterViewModel alloc] initWithServices:self.services params:nil];
        }
    ...
    self.captchaCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
        ...
        return [[[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            ...
            @strongify(self);
            self.count = FSLCaptchaFetchMaxWords;
...
#pragma mark - 辅助方法
- (RACSignal *) _fetchUserInfoWithUserLoginChannel:(FSLUserLoginChannelType)channel{
    @weakify(self);
    return [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {

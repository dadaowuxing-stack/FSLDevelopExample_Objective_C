...
@interface FSLTableViewController ()
/// tableView
@property (nonatomic, readwrite, weak)   FSLTableView *tableView;
/// contentInset defaul is (64 , 0 , 0 , 0)
@property (nonatomic, readwrite, assign) UIEdgeInsets contentInset;
/// 视图模型
@property (nonatomic, readonly, strong) FSLTableViewModel *viewModel;
@end

@implementation FSLTableViewController
...
/// init
- (instancetype)initWithViewModel:(FSLTableViewModel *)viewModel {
...
- (void)_su_setupSubViews{
    ...
    if (self.viewModel.shouldPullDownToRefresh) {
        /// 下拉刷新
        @weakify(self);
        [self.tableView fsl_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            /// 加载下拉刷新的数据
            @strongify(self);
    ...
    if (self.viewModel.shouldPullUpToLoadMore) {
        /// 上拉加载
        @weakify(self);
        [self.tableView fsl_addFooterRefresh:^(MJRefreshAutoNormalFooter *footer) {
            /// 加载上拉刷新的数据
            @strongify(self);

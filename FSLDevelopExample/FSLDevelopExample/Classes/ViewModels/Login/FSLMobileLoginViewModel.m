...
@interface FSLMobileLoginViewModel ()
/// closeCommand
...
@end
@implementation FSLMobileLoginViewModel
- (instancetype)initWithServices:(id<FSLViewModelServices>)services params:(NSDictionary *)params
{
    if (self = [super initWithServices:services params:params]) {
        self.phone = params[FSLMobileLoginPhoneKey];
        self.zoneCode = params[FSLMobileLoginZoneCodeKey];
    }
    ...
    self.validLoginSignal = [[RACSignal
                              ...
                              reduce:^(NSString *password , NSString *captcha , NSNumber *selected) {
                                  if (selected.boolValue) {
                                      return @(FSLStringIsNotEmpty(captcha));
                                  }else{
                                      return @(FSLStringIsNotEmpty(password));
                                  }
                              }]
...
- (RACSignal *) _fetchUserInfo{
    ...
    return [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            FSLUser *user = [[FSLUser alloc] init];
            ...
            user.coverImage = FSLImageNamed(@"Kris.jpeg");

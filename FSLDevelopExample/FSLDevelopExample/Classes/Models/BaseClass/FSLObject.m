...
#import "FSLObject.h"
#import "FSLReflection.h"
#import "FSLEXTRuntimeExtensions.h"
#import "NSError+ModelException.h"

// Used to cache the reflection performed in +propertyKeys.
static void *FSLObjectCachedPropertyKeysKey = &FSLObjectCachedPropertyKeysKey;
...
#else
        if (error != NULL) {
            *error = [NSError FSL_modelErrorWithException:ex];
        }
...
+ (NSSet *)propertyKeys {
    NSSet *cachedKeys = objc_getAssociatedObject(self, FSLObjectCachedPropertyKeysKey);
    if (cachedKeys != nil) return cachedKeys;
    
    NSMutableSet *keys = [NSMutableSet set];
    
    [self enumeratePropertiesUsingBlock:^(objc_property_t property, BOOL *stop) {
        fsl_propertyAttributes *attributes = fsl_copyPropertyAttributes(property);
        @onExit {
            free(attributes);
        };
    ...
    objc_setAssociatedObject(self, FSLObjectCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
    ...
    while (!stop && ![cls isEqual:FSLObject.class]) {
        unsigned count = 0;
...
#pragma mark Merging
- (void)mergeValueForKey:(NSString *)key fromModel:(FSLObject *)model {
    NSParameterAssert(key != nil);
    
    SEL selector = FSLSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
...
- (void)mergeValuesForKeysFromModel:(FSLObject *)model {
